...
def main() -> None:
    ...
    print(f"  Mean absolute gap: {statistics.mean(abs_gaps)*100:.1f}¢")
    print(f"  Median absolute gap: {statistics.median(abs_gaps)*100:.1f}¢")
    print(f"  Max absolute gap: {max(abs_gaps)*100:.1f}¢")
    print(f"  Mean signed gap (model - Kalshi): {statistics.mean(signed_gaps)*100:+.1f}¢")
    print(f"  Std dev of gap: {statistics.stdev(signed_gaps)*100:.1f}¢")
    ...
    if abs(statistics.mean(signed_gaps)) > 0.03:
        ...
        print(f"    by {abs(statistics.mean(signed_gaps))*100:.1f}¢ on average")
    else:
        print(f"\n  → No strong systematic bias (mean gap within ±3¢)")
    ...
    for goal_min, description, team in goal_info:
        ...
        if pre_model is not None and post_model is not None:
            ...
            print(f"    Model:  {pre_model:.3f} → {post_model:.3f} (Δ = {model_adj:+.3f} = {model_adj*100:+.1f}¢)")
        else:
            model_adj = None
        ...
        if pre_kalshi is not None and post_kalshi is not None:
            ...
            print(f"    Kalshi: {pre_kalshi:.3f} → {post_kalshi:.3f} (Δ = {kalshi_adj:+.3f} = {kalshi_adj*100:+.1f}¢)")
        else:
            kalshi_adj = None
        ...
        if model_adj is not None and kalshi_adj is not None:
            ...
            print(f"    Adjustment gap: {diff:+.3f} ({diff*100:+.1f}¢)")
    ...
    if late_gaps:
        print(f"\n  Mean |gap| min 78-90: {statistics.mean([g['abs_gap'] for g in late_gaps])*100:.1f}¢")
    if stoppage_gaps:
        print(f"  Mean |gap| min 90+:   {statistics.mean([g['abs_gap'] for g in stoppage_gaps])*100:.1f}¢")
    ...
    for m in check_minutes:
        ...
        if model:
            ...
            print(f"  Remaining μ: μ_H={model['mu_H']:.3f}, μ_A={model['mu_A']:.3f}")
            print(f"  Sanity: μ_H+μ_A = {model['mu_H']+model['mu_A']:.3f} "
        ...
        if m == 40:
            ...
            if model["p_home"] > 0.80:
                ...
                print(f"  ✗ Seems too low — model may be underreacting to 2-goal lead")
    ...
    for phase_name, phase_gaps in phases.items():
        ...
        print(f"  {phase_name:<30} {len(phase_gaps):>3} "
              f"{statistics.mean(ag)*100:>8.1f}¢ {statistics.mean(sg)*100:>+8.1f}¢ "
              f"{max(ag)*100:>8.1f}¢")

    # ── Step 10: Old vs New comparison ───────────────────────────────────────
    print("\n" + "=" * 72)
    print("OLD (6-PERIOD) vs NEW (8-PERIOD) COMPARISON")
    print("=" * 72)

    # Previous results from 6-period model
    old_mean_gap = 3.4
    old_late_gap = 6.9
    new_mean_gap = statistics.mean(abs_gaps) * 100

    late_game_entries = [g for g in gaps if 78 <= g["minute"] <= 90]
    new_late_gap = statistics.mean([g["abs_gap"] for g in late_game_entries]) * 100 if late_game_entries else 0.0

    stoppage_entries = [g for g in gaps if g["minute"] > 90]
    new_stoppage_gap = statistics.mean([g["abs_gap"] for g in stoppage_entries]) * 100 if stoppage_entries else 0.0

    print(f"""
  {'Metric':<35} {'Old (6-period)':>14} {'New (8-period)':>14} {'Delta':>10}
  {'-'*75}
  {'Mean |gap| (all minutes)':<35} {old_mean_gap:>13.1f}¢ {new_mean_gap:>13.1f}¢ {new_mean_gap - old_mean_gap:>+9.1f}¢
  {'Late-game |gap| (min 78-90)':<35} {old_late_gap:>13.1f}¢ {new_late_gap:>13.1f}¢ {new_late_gap - old_late_gap:>+9.1f}¢
  {'Stoppage |gap| (min 90+)':<35} {'N/A':>14} {new_stoppage_gap:>13.1f}¢ {'(new)':>10}""")
    ...
    print(f"""
  OVERALL ACCURACY:
    Mean |gap|: {mean_abs:.1f}¢ (model vs Kalshi mid-price)
    Mean bias: {mean_signed:+.1f}¢ ({'model too high' if mean_signed > 0 else 'model too low'})
    Max gap: {max_gap_entry['abs_gap']*100:.1f}¢ at min {max_gap_entry['minute']} ({max_gap_entry['score'][0]}-{max_gap_entry['score'][1]})

  INTERPRETATION:
    - Mean |gap| < 5¢: Model is competitive with market
    - Mean |gap| 5-10¢: Model has significant calibration work needed
    - Mean |gap| > 10¢: Model is fundamentally mispriced
  ...
  BIGGEST GAP SITUATIONS:""")
    ...
    if post_goal_gaps:
        print(f"    Post-goal minutes: mean |gap| = {statistics.mean([g['abs_gap'] for g in post_goal_gaps])*100:.1f}¢")
    if normal_gaps:
        print(f"    Normal play minutes: mean |gap| = {statistics.mean([g['abs_gap'] for g in normal_gaps])*100:.1f}¢")
